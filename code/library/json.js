(function() {
	...
	var projectID = Scratch.INIT_DATA.PROJECT.model.id,
		json = JSON.parse(localStorage.json)[projectID] || {},
		editor = Scratch.FlashApp.ASobj;
	$('head').append('<style type="text/css">.json-no-select{-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;-o-user-select:none;user-select:none}.json-dialog{position:fixed;left:0;top:0;z-index:9999;color:#5c5d5f;text-align:center;border-radius:7px 7px 0 0;box-shadow:3px 3px 8px rgba(51,51,51,.75);text-shadow:none}.json-dialog-title{display:block;border:1px solid #b0b0b0;border-radius:7px 7px 0 0;background:linear-gradient(#e0e0e0,#d0d0d0);font-size:14px;height:30px;line-height:30px;padding:0 14px;cursor:default}.json-dialog-content{background:#fff;padding:14px;border:1px solid #b0b0b0;border-top:0;font-size:14px;line-height:normal}.json-dialog .iframeshim{border-radius:7px 7px 0 0}.json-dialog-label{display:block;margin-bottom:14px;font-size:inherit;line-height:inherit;color:inherit}.json-dialog-buttons{margin-top:14px}button.json-ui-button{min-width:51px;margin:0 5px;border:1px solid #d0d1d2;border-radius:6px;padding:0 7px 1px;height:26px;background:linear-gradient(#fff,#e6e8e8);color:#5c5d5f;font:inherit;font-size:12px;white-space:nowrap;box-shadow:none;transition:none;text-shadow:none;cursor:default}button.json-ui-button:hover{background:#179fd7;color:#fff;font-weight:400}.json-dialog-field{margin:0 0 0 8px;padding:4px 1px;border:1px solid #d0d1d2;border-radius:0;background:#fff;font:inherit;font-size:12px;outline:0;-webkit-appearance:none;display:inline-block;width:auto;height:auto;box-shadow:none}</style>');
	...
	function Dialog(title, content) {
		this.el = el('json-dialog json-no-select');
		this.el.appendChild(this.elTitle = el('json-dialog-title'));
		this.el.appendChild(this.elContent = content || el('json-dialog-content'));
		if (content) content.classList.add('json-dialog-content');
	...
	Dialog.label = function(text) {
		var div = el('json-dialog-label');
		div.textContent = text;
		return div;
	};
	Dialog.Field = function(label, value) {
		this.value = '';
		this.el = el('label', 'json-dialog-label');
		this.el.textContent = label;
		this.field = el('input', 'json-dialog-field');
	...
	Dialog.buttons = function() {
		var div = el('json-dialog-buttons');
		var a = [].slice.call(arguments);
		for (var i = 0, l = a.length; i < l; i++) {
			var b = a[i];
			if (typeof b !== 'object') b = [b, b];
			var button = el('button', 'json-ui-button');
	...
	extBase._shutdown = function() {
        var cookieVarBank = JSON.parse(localStorage.cookieLists);
		cookieVarBank[projectID] = cookieVars;
		localStorage.cookieLists = JSON.stringify(cookieVarBank);
	};
	extBase._getStatus = function() {
		return {status: 2, msg: 'Ready'};
	};
	extBase.makeCookieVar = function() {
		var name = new Dialog.Field('List name:');
		var d = new Dialog('New JSON list', Dialog.content(
			name.el,
			Dialog.buttons(
		...
		d.oncommit = function() {
			if (name.value in json) Dialog.alert('Cannot Add', 'That name is already in use.').show(editor)
			else {
				json[name.value] = 0;
				reloadExtension();
			}
	...
	extBase.deleteCookieVar = function() {
		var name = new Dialog.Field('List name:');
		var d = new Dialog('Delete a JSON list', Dialog.content(
			name.el,
			Dialog.buttons(
				['OK', function() {d.commit()}],
				['Cancel', function() {d.hide()}]
		...
		d.oncommit = function() {
			if (!(name.value in json)) Dialog.alert('Cannot Delete', 'An JSON list with that name does not exist.').show(editor)
			else {
				delete json[name.value];
				reloadExtension();
			}
		};
		d.show(editor);
    ...
    extBase.set = function(item, name, value) {
        try {
            name = name.substr(3);
    ...
    extBase.get = function(item, name, value, callback) {
        try {
            name = name.substr(3);
            callback(json[name][item]);
        } catch(e) {
            callback('');
        }
    }
	...
	function loadExtension() {
		var ext = Object.create(extBase),
			varNames = Object.getOwnPropertyNames(json),
			descriptor = {};
		var db = new DescriptorBuilder(descriptor);
		db.addButton('Make a cookie list', 'makeCookieVar');
		if (varNames.length > 0) {
			var defaultVar = '\ud83c\udf6a ' + varNames[varNames.length - 1],
				varName, escapedVarName;
			db.addButton('Delete a cookie list', 'deleteCookieVar');
			db.addSpace();
			for (var i = 0; i < varNames.length; i++) {
				varName = varNames[i];
				escapedVarName = varName.replace(/\\/g,'\\\\').replace(/'/g,"\\'").replace(/\n/g, '\\n');
				db.addBlock('r', '△ ' + varName, 'getCookieVar:' + varName);
				ext['getCookieVar:' + varName] = (new Function("return this['" + escapedVarName + "']")).bind(json);
			}
            
            // block definitions
			db.addSpace();
			db.addBlock(' ', 'set item %n of %m.cookieVar to %s', 'set', [1, defaultVar, 'thing']);
			db.addBlock('R', 'item %n of %m.cookieVar', 'get', [1, defaultVar]);
            db.addBlock('R', 'length of %m.cookieVar', 'getLength', [defaultVar]);
		...
		ScratchExtensions.register('Cookie Lists', descriptor, ext);
	}
	
	function reloadExtension() {
		ScratchExtensions.unregister('Cookie Lists');
		loadExtension();
	}
